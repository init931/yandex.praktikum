...
public:
    ...
    //
    explicit Witch(Witch&&) = default;
    Witch& operator=(Witch&&) = default;

    explicit Witch(const Witch& obj)
            : name_(obj.GetName()),
            cat_(make_unique<Cat>(obj.cat_->GetName(), obj.cat_->GetAge())) {
                
    }
    
    Witch& operator=(Witch& obj) {
        Witch t(obj);
        name_ = t.GetName();
        //std::unique_ptr<Cat> released_cat = t.ReleaseCat();
        SetCat(t.ReleaseCat());
        return *this;
    }
    //
