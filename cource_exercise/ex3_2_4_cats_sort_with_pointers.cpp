...
void PrintCatPointerValues(const vector<const Cat*>& cat_pointers, ostream& out) {
    ...
    //for (const Cat* item : cat_pointers) {
    //    out << *item << endl;
    //}
...
int main() {
    ...
    {
        const vector<Cat> cats_empty = {};
        auto sorted_cats = GetSortedCats(cats_empty, [](const Cat& lhs, const Cat& rhs) {
            return tie(lhs.gender, lhs.breed) < tie(rhs.gender, rhs.breed);
        });
    ...
    {
        const vector<Cat> cats_one = {
            {"Tom"s, Gender::Male, CatBreed::Bengal, 2}
        };
        auto sorted_cats = GetSortedCats(cats_one, [](const Cat& lhs, const Cat& rhs) {
            return tie(lhs.gender, lhs.breed) < tie(rhs.gender, rhs.breed);
        });

        cout << "One element vector:"s << endl;
        PrintCatPointerValues(sorted_cats, cout);
        cout << endl;
    }
}
