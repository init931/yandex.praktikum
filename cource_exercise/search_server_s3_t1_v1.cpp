...
public:
    ...
    [[nodiscard]] bool AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        if (document_id < 0) {
            //нельзя добавлять документы с отрицательным идентификатором
            return false;
        }

        if (documents_.find(document_id) != documents_.end()) {
            //документ с таким идом уже существует
            return false;
        }
        ...
        for (const string& word : words) {
            if (!IsValidWord(word)) {
                //вернуть инфу что ошибка именно в наличии спецсимволов
                return false;
            }
        }
    ...
    [[nodiscard]] bool FindTopDocuments(
        ...
        //опасно переопределять сигнатуру метода ParseQuery т к тренажер непредстказуем
        //пусть лучше будет дублирование for
        for (const string& word : SplitIntoWords(raw_query)) {
            ...
            if (!IsValidMinusWord(word)) {
                //вернуть инфу что ошибка именно в неправильно введеных минус словах
                return false;
            }
        }
