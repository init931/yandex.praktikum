...
public:
    ...
    void Withdraw(const VehiclePlate& car) {
        ...
        if (now_parked_.count(car) == 0) {
            ...
            auto diff = Clock::now() - now_parked_.at(car);
            complete_parks_[car] = diff;
    ...
    int64_t GetCurrentBill(const VehiclePlate& car) const {
        ...
        if (complete_parks_.find(car) != complete_parks_.end()){
            Duration t = complete_parks_.find(car)->second;
            secs += duration_cast<chrono::seconds>(t).count();
        }
        if (now_parked_.find(car) != now_parked_.end()) {
            Duration t = Clock::now() - now_parked_.find(car)->second;
            secs += duration_cast<chrono::seconds>(t).count();
        }
        int64_t bill = cost_per_second_ * secs;
        return bill;
    }
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        // место для вашей реализации
        std::unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> ret;
        std::for_each(now_parked_.begin(), now_parked_.end(), [&ret, this](auto& item) {
            auto secs = duration_cast<chrono::seconds>(Clock::now() - item.second).count();
            ret[item.first] = secs * cost_per_second_;
            item.second = Clock::now();
        });
        std::for_each(complete_parks_.begin(), complete_parks_.end(), [&ret, this](auto& item) {
            if (item.second.count() > 0) {
                auto secs = duration_cast<chrono::seconds>(item.second).count();
                ret[item.first] += secs * cost_per_second_;
                item.second = {};
            }
        });
