...
public:
    ...
    Iterator InsertAfter(ConstIterator pos, const Type& value) {
		...
		auto n = new Node(value, next_node);
        auto it = Iterator(n);
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        //copy PopFront()
        auto t = pos.node_->next_node;
		pos.node_->next_node = t->next_node;
        auto it = Iterator(pos.node_->next_node);
		delete t;
		--size_;
		return it;
    }
    ...
    void swap(SingleLinkedList& other) noexcept {
		...
		Node* otherHeadNode = other.head_.next_node;
		other.head_.next_node = head_.next_node;
		head_.next_node = otherHeadNode;
    }
    ...
    [[nodiscard]] ConstIterator cbefore_begin() const noexcept {
        auto cc = const_cast<Node*>(&head_);
        return ConstIterator(cc);
    }
	...
	void copyFrom_(const T& values) {
		...
		try {
			for (const auto& item : values) {
				(*last_elem).next_node = new Node(item, nullptr);
...
void Test1() {
    ...
    // Проверка фактического удаления элементов
    {
        ...
        //std::cout << item0_counter << std::endl;
        assert(item0_counter == 0);
        assert(item1_counter == 0);
        assert(item2_counter == 0);
    }
