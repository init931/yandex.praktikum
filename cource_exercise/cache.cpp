...
public:
    ...
    std::shared_ptr<Value> GetValue(const Key& key) {
        ...
        std::shared_ptr<Value> obj = cache_[key].lock();
        if (!obj) {
            obj = value_factory_(key);
            cache_.at(key) = { obj };
        }
...
public:
    ...
    explicit BookLoader(const BookStore& store, size_t& load_count)
            : storage_(store), load_count_(load_count) {
        // Реализуйте конструктор самостоятельно
    }
    ...
    std::shared_ptr<Book> operator()(const std::string& title) const {
        ...
        if (storage_.find(title) == storage_.end()) {
            throw std::out_of_range("Not implemented");
        }
        else {
            load_count_++;
            Book book = { title, storage_.at(title) };
